...
class PowerMeter: AudioLevelProvider {
    ...
    var levels: AudioLevels {
        ...
        return AudioLevels(
            level: meterTableAverage.valueForPower(values[0].average),
    ...
    func process(buffer: CMSampleBuffer) {
        let averagePower = buffer.toDBFS()
        let peakPower = buffer.toPeakDBFS()
        let powerLevel = PowerLevels(
            average: averagePower,
            peak: peakPower
        )

        var powerLevels = [PowerLevels]()
        powerLevels.append(powerLevel)

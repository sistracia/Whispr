...
final class AudioProcessController {
    ...
    private(set) var appAudios = [AudioProcess]()
    private(set) var processAudios = [AudioProcess]()
    ...
    fileprivate func reload(apps: [NSRunningApplication]) {
        ...
        do {
            ...
            let processes = updatedProcesses
            ...
            self.setProcessAudios(processes)
        } catch {
            logger.error("Error reading process list: \(error, privacy: .public)")
        }
    }
    
    private func setProcessAudios(_ processes: [AudioProcess]) {
        var appAudios = [AudioProcess]()
        var processAudios = [AudioProcess]()
        
        for process in processes {
            switch process.kind {
                case .app:
                appAudios.append(process)
                break
            case .process:
                processAudios.append(process)
                break
            }
        }
        
        self.appAudios = appAudios
        self.processAudios = processAudios
    }
